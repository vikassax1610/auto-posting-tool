...
const AccountsPage = () => {
  ...
  const handleConnect = async (platform, label) => {
    setActionLoading(platform);
    try {
      const response = await connectAccount(platform);

      // Handle OAuth Redirect URL if returned from backend
      ...
      toast.error(error.response?.data?.message || `Failed to connect ${label}`);
    } finally {
      setActionLoading(null);
    }
  };

  const handleDisconnect = async (platform, label) => {
    setActionLoading(platform);
    try {
      ...
      toast.error(error.response?.data?.message || `Failed to disconnect ${label}`);
    } finally {
      setActionLoading(null);
    }
  };
      ...
      <div className="flex flex-col gap-5 border-b border-border pb-6 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="inline-flex w-fit items-center gap-2 rounded-xl border border-border bg-card px-4 py-3">
          ...
          <span className="text-sm font-semibold text-white">
            {loading ? 'Checking connections' : `${connectedCount} of ${platformCards.length} connected`}
          </span>
        </div>
      </div>
      ...
      {loading ? (
        <div className="grid gap-5 md:grid-cols-2">
          {platformCards.map(({ id }) => (
            <Card key={id} className="min-h-[266px] p-6 animate-pulse">
            ...
            return (
              <Card key={id} hover className="flex min-h-[266px] flex-col p-6">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span
                    className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-semibold ${isConnected
                ...
                <div className="mt-5 rounded-xl border border-border bg-bg px-4 py-3">
                  ...
                  <p className="mt-1 truncate text-sm font-semibold text-white">
                    {isConnected ? connectionName : 'No account connected'}
                  </p>
                  <p className="mt-1 truncate text-xs text-muted">
                    {isConnected && account.accountId ? `ID: ${account.accountId}` : 'Connection required before publishing'}
                  </p>
                </div>
                  ...
                  {isConnected ? (
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        ...
                        disabled={isLoading}
                        className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-primary/50 px-3 text-sm font-semibold text-primary transition-colors hover:bg-primary/10 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        {isLoading ? <Loader size="sm" /> : <RefreshCw size={16} />}
                        Reconnect
                      </button>
                      <button
                        type="button"
                        ...
                        disabled={isLoading}
                        className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-border px-3 text-sm font-semibold text-muted transition-colors hover:border-error/60 hover:text-error disabled:cursor-not-allowed disabled:opacity-50"
                      >
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-warning/40 bg-warning/10 p-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-muted">
              Your posts will only be sent to channels that you explicitly connect here.
            </p>
          </div>
        </div>
      )}
